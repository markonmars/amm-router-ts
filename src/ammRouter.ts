...
import { calculateOutputXYKPool, calculateRequiredInputXYKPool } from './helpers/math'
import { RouteHop } from './types/routeHop'
import { Pool } from './types/pool'
import Long from 'long'
import { Converter } from './converters/converterInterface'
...
export class AMMRouter implements AMMRouterInterface {
	...
	setPools(pools: any[], converter: Converter) {
		this.pools = converter.generatePools(pools)
	}
	...
	getBestRouteGivenInput(
		...
		return this.getRouteWithHighestOutput(new BigNumber(amountIn), routeOptions)
	}
	...
	getBestRouteGivenOutput(
		...
		const routeOptions = this.getRoutes(tokenInDenom, tokenOutDenom)
		return this.getRouteWithLowestInput(new BigNumber(amountOut), routeOptions)
	}
